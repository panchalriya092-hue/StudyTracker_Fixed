...
class StudyVisualizer:
    ...
    def plot_subject_distribution(tracker: StudyTracker) -> plt.Figure:
        ...
        # FIXED: Filter out subjects with zero hours
        filtered_data = [(s, h) for s, h in zip(subjects, hours) if h > 0]
        
        if not filtered_data:
            ax.text(0.5, 0.5, 'No study sessions recorded yet\n\nAdd some sessions to see the chart!', 
                   ha='center', va='center', fontsize=12)
            ax.axis('off')
            return fig
        
        subjects_filtered = [d[0] for d in filtered_data]
        hours_filtered = [d[1] for d in filtered_data]
        
        colors = plt.cm.Set3(np.linspace(0, 1, len(subjects_filtered)))
        
        ax.pie(hours_filtered, labels=subjects_filtered, autopct='%1.1f%%', colors=colors, startangle=90)
    ...
    def plot_study_timeline(tracker: StudyTracker, days: int = 30) -> plt.Figure:
        ...
        # Add statistics
        if np.sum(daily_time) > 0:  # Only if there's data
    ...
    def create_summary_dashboard(tracker: StudyTracker) -> plt.Figure:
        ...
        if tracker.subjects:
            ...
            if filtered_data:
                ...
                hours_filtered = [d[1] for d in filtered_data]
                colors = plt.cm.Set3(np.linspace(0, 1, len(subjects_filtered)))
                ax1.pie(hours_filtered, labels=subjects_filtered, autopct='%1.1f%%', colors=colors)
        ...
        if subjects_with_goals:
            ...
            ax4.text(0.5, 0.5, 'No goals set', ha='center', va='center')
            ax4.axis('off')
